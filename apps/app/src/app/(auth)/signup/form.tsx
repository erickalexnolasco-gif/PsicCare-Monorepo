  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border font-medium text-sm transition-all hover:bg-white/60 shadow-sm"
        style={{ borderColor: "var(--psi-border)", background: "rgba(255, 255, 255, 0.4)" }}
      >
        {isGooglePending ? "Conectando con Google..." : (
          <>
      ...
      <div className="flex items-center gap-3">
        ...
        <span className="text-xs uppercase tracking-wider" style={{ color: "var(--psi-soft)" }}>o con correo</span>
        <div className="flex-1 h-px" style={{ background: "var(--psi-border)" }} />
      </div>
      ...
      >
        ...
        <button type="submit" disabled={isPending || isGooglePending} className="btn-primary w-full justify-center py-3" data-testid="signup-submit">
          {isPending ? "Creando cuenta..." : "Crear cuenta gratis"}
        </button>
        
        <div className="text-center text-sm pt-2" style={{ color: "var(--psi-soft)" }}>
          ¿Ya tienes cuenta? <Link href="/login" className="font-medium hover:underline transition-all" style={{ color: "#D88AAB" }}>Inicia sesión</Link>
        </div>
      </form>
    </div>
  );
